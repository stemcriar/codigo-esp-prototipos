...
void WifiController::startWebServer(String id) 
{
  ...
  server.on("/", [this]() {
    ...
    html += "<input type='text' name='novo_ip' placeholder='Digite o novo IP. Ex: 192.168.15.8' required>";
    html += "<button type='submit'>Salvar IP</button>";
    html += "</form>";

    html += "<hr>";

    html += "<h3>Rede Wi-Fi</h3>";
    html += "<p>Conectado atualmente em: <br><strong style='color:#2196F3; font-size:1.2em;'>" + redeAtual + "</strong></p>";
    html += "<p style='font-size: 0.9em; color: #666;'>Deseja conectar a outra rede?</p>";
    html += "<form action='/reset_wifi' method='POST'>";
    html += "<button type='submit' class='btn-danger'>Esquecer rede atual</button>";
    html += "</form>";
  ...
  server.on("/reset_wifi", HTTP_POST, [this]() {
    String html = "<html><head><meta charset='UTF-8'>";
    html += "<meta name='viewport' content='width=device-width, initial-scale=1.0'>";
    html += "<style>body{font-family:sans-serif; text-align:center; padding: 20px;} .destaque{color:#f44336;} ";
    html += ".btn { display: inline-block; padding: 12px 24px; margin-top: 20px; background-color: #2196F3; color: white; text-decoration: none; border-radius: 5px; font-weight: bold; }";
    html += ".alerta { margin-top: 30px; padding: 15px; background-color: #fff3cd; color: #856404; border: 1px solid #ffeeba; border-radius: 5px; font-size: 0.9em; text-align: left; }</style></head><body>";
    
    html += "<h2 class='destaque'>Wi-Fi Redefinido!</h2>";
    html += "<p>O protótipo esqueceu a rede atual.</p>";
    html += "<p>Para escolher uma nova rede, conecte-se ao Wi-Fi <br><strong style='font-size:1.2em;'>Config-ESP-" + this->id + "</strong><br> no seu dispositivo.</p>";
    
    html += "<a href='/' class='btn'>Voltar à Página Inicial</a>";

    html += "<div class='alerta'>";
    html += "<h3 style='margin-top: 0;'>⚠️ Ação Necessária</h3>";
    html += "<p>Para que a rede de configuração apareça, <strong>desligue e ligue " + this->id + "</strong>.</p>";
    html += "</div>";
